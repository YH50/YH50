...
.container{width:560px; margin:0 auto; border:1px solid gray; display:flex;
...
<%
	...
	// 최초 실행 시 request.getParameter("sYear") 는 null 값
	// 이전 달이나 다음 달 버튼을 클릭하고 이동해온 경우엔 null 도 아니고 그 값으로 sYear와 sMonth 를 교체해줘야 함.
	...
	sDay.set(sYear, sMonth, 1);		// sDay 객체를 현재 달 1일로 수정
	int lastDay = sDay.getActualMaximum(Calendar.DATE);		// 현재 달의 말일 추출을 저장
	int start_week = sDay.get(Calendar.DAY_OF_WEEK);			// 현재 달 1일의 요일을 추출
	
%>
<div class="title">
	<input type = "button" value = "이전달" onClick="location.href='07_Calendar.jsp?sYear=<%=sYear %>&sMonth=<%=sMonth-1%>'"/>&nbsp;
	<%
	out.print("<span style = 'font-size:180%; font-weight:bold'>" + sYear + "년" + (sMonth + 1) + "월</span>");
	%>
	&nbsp; <input type = "button" value = "다음달" onClick="location.href='07_Calendar.jsp?sYear=<%=sYear%>&sMonth=<%=sMonth+1%>'"/>
</div>
<div class="container">
	<div class="row">
		<div class="column">일</div><div class="column">월</div><div class="column">화</div>
		<div class="column">수</div><div class="column">목</div><div class="column">금</div>
...
<%
	...
	int k;
	for(k=1;k<start_week;++k)		//첫 칸부터 1일자 요일 칸 이전까지 빈칸으로 대체
